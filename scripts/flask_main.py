...
def processDatabase(filename: str):
    ...
    while row is not None and (idx <= GEOLIM or not GEOLIM):
        ...
        cmdstr = "UPDATE locations SET isState='%s', communityName='%s', geojson='%s' WHERE rowid=%d" % ("FALSE", city, str(geographicalJson), rowid)
        commandQueue.append(cmdstr)

        # Generate state data
        print("Generating state data from {0:s}".format(DBFOLDER + filename))
